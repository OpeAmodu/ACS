...
The data set contains only people who are current and former smokers. 
...
nhis18_modelling$ORIENT_A <- factor(nhis18_modelling$ORIENT_A, 
                                    levels = c("Straight", "Gay/Lesbian", "Bisexual",
                                               "Something else", "Don't know the answer"))
...
Survey weights was applied throughout the analysis as specified below following the recommendation from NHIS. Link to recommendation can be found [here](https://www.cdc.gov/nchs/nhis/nhis_2018_data_release.htm).
...
Total observations, variables collected are reported below.
```{r}
str(nhis18_modelling)
```

### $\text{\underline{Top Years People Quit Smoking}}$
...
```
\newpage
### $\text{\underline{Summary of Study Sample by Subpopulation (smokers and quitters)}}$
The function I created below summarizes the data by different sub-populations and reports the Frequency and percentage in a table.
```{r}

freq_table_function <- function(my_data, a_var, label){
...
\newpage
## $\text{\underline{Creating datasets for each types of quit }}$
...
\newpage
## $\text{\underline{REGRESSION ANALYSIS}}$


### *Creating a function that calculates un-adjusted and adjusted Quit rates*

The function I defined below calculates un-adjusted rates using the original data and uses the regression results to predict the quit probability for each observation in the data set. The quit probability is summarized across different sub-populations and reported with the un-adjusted rate in a table as a percentage. See code chunk below for function. I implemented this function for each regression analysis to show the table for adjusted and un-adjusted quit/attempt to quit probabilities.  
```{r, echo=TRUE}
...
\newpage
### $\text{\underline{QUIT SMOKING FOR LESS THAN A YEAR}}$
...
```
\newpage
### $\text{\underline{QUIT SMOKING FOR AT LEAST 6 MONTHS}}$
```{r}
...
\newpage
### $\text{\underline{QUIT SMOKING FOR AT LEAST A YEAR}}$
```{r}
...
\newpage
### $\text{\underline{ATTEMPTS TO QUIT SMOKING}}$
```{r}

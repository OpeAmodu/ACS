...
knitr::opts_chunk$set(cache=FALSE, warning=FALSE, message=FALSE, echo=FALSE, knitr.table.format="latex",
                      fig.pos = "H", out.width="100%", dpi=300)
```
...
Analyses was done using R version 4.0.4 (2021-02-15) -- "Lost Library Book" Copyright (C) 2021 and Rstudio Version 1.4.1106 © 2009-2021 RStudio, PBC. Installers and instructions for R can be found at the [cran website link](https://cran.rstudio.com/). To install Rstudio, please click this [R studio link](https://rstudio.com/products/rstudio/download/). Analyses depends on installing the latest version of the following R packages:
...
#These libraries are essential for the analysis in the markdown to work
...
```{r}
#To make the resulting tables clearer down the line, I renamed some of the 
#columns to they are easier to read when we produce results
...
For regression modeling, we only included observations where the respondents identified as former or current smokers. The resulting data set had `r nrow(nhis18_modelling)` observations after adjusting for age and smoking status.
...
# This code produces a table for the top 10 years people have quitted smoking
# It calculates the exact year each person in the data set quit smoking and summates
#the years to know which years appeared the most
...
\newpage
<!--
The freq_table_function calculates the frequency of each groups of a variable in 
percent. For example, the function the education variable by its categories.
-->
```{r}
...
<!--
The code chunks below are different subsets of the data with QUIT_TYPE being the 
outcome variable.
- 0 means the person did not quit smoking
- 1 means that the person quit smoking
- For each sub data set, I built a new survey design 
- I also made a data set for attempt to quit smoking
- Not all data sets was used in the analysis
-->
...
<!--
The readjust_function function creates an un_adjusted and adjusted quit rates table for 
quit rates by socio-economic and socio-demographic variables.

The general_quit_function is similar to the readjust function. However, the general quit function
creates a table that calculates the total unadjusted and adjusted quit rates 

After using the general_quit function and the readjust_function to make small tables, 
I binded the tables to combine all the tables into one large table

Then I used kable_extra to design the final outputed tables for each outcome
...
## Recent Successful Quit
```{r, log regression for recent successful quit}
...
```{r, summary of results for recent succesful quit}
#summary(reg1)
...
kable(summary, caption = "Adjusted Odds Ratio estimates for Recent Successful Quits", booktabs=T, digits=3, linesep="\\addlinespace",
      longtable=T, align="crrr")%>%
  kable_styling(latex_options =c("HOLD_position", "repeat_header"),full_width=T, font_size = 9)%>%
  footnote(number=c("Variables are statistically significant at alpha = 0.1"))
```
...
kable(adjust_table, caption = "Adjusted and Unadjusted Quit Rates for Recent Successful Quit", booktabs=T, longtable=T, align = "lrr")%>%
  ...
  kable_styling(latex_options=c("striped", "HOLD_position", "repeat_header"), full_width=T, font_size = 8.5)
```
...
summary <- summary %>%
  select(term, `Adjusted odds ratio`=estimate, statistic, `p value` =p.value)
kable(summary, caption = "Adjusted Odds Ratio estimates for at least a Year", booktabs=T, digits=3, linesep="\\addlinespace",
      longtable=T, align = "crrr")%>%
  kable_styling(latex_options =c("HOLD_position", "repeat_header"),full_width=T, font_size = 8.5)%>%
  footnote(number=c("Variables are statistically significant at alpha = 0.1"))
```
...
adjust_table <- rbind(general_quit, region, age, sex, education, ratcat, mental_health, race, houseown, income, food_sec, orient, coverage)
kable(adjust_table, caption = "Adjusted and Unadjusted Quit Rates for at least a year", booktabs=T, align="lrr")%>%
  pack_rows(index=c(" " = 1, "Region"= 4, "Age"=3, "Sex"=2, "Education"=4, "Ratio of Income to FPL"=4, "Mental health problem"=2,
                    "Race"=3, "Home Ownership"=2, "Income Group"=4, "Food Security"=2, "Sexual Orientation"=2,
                    "Health insurance coverage"=5))%>%
  kable_styling(latex_options=c("HOLD_position","striped"), full_width=T, font_size = 9)

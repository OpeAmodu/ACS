...
#
cigar_by_ageDF <- cigar_by_age %>% 
  as.data.frame()%>%
  group_by(binned_age)%>%
  gather(`CIGARST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
  mutate(Prevalence = (Yes/(Yes + No))*100)
cigar_by_ageDF$`Type of Smoke` <- ifelse(cigar_by_ageDF$`Type of Smoke` == "CIGARST_A", "Cigar", "")
#
pipe_by_ageDF <- pipe_by_age %>% 
  as.data.frame()%>%
  group_by(binned_age)%>%
  gather(`PIPEST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
  mutate(Prevalence = (Yes/(Yes + No))*100)
pipe_by_ageDF$`Type of Smoke` <- ifelse(pipe_by_ageDF$`Type of Smoke` == "PIPEST_A", "Pipe", "")
#
smkles_by_ageDF <- smkles_by_age %>% 
  as.data.frame()%>%
  group_by(binned_age)%>%
  gather(`SMOKELSST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
  mutate(Prevalence = (Yes/(Yes + No))*100)
...
kable(smoking_by_age, caption = "Table Showing the Frequency of Smoking by Age and Type of Product")
...
kable(smoking_by_sex, caption = "Table Showing the Frequency of Smoking by Sex and Type of Product")
...
#
smkles_by_raceDF <- smkles_by_race %>% 
  as.data.frame()%>%
  group_by(RACEALLP_A)%>%
  gather(`SMOKELSST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
...
```{r}
cig_by_educDF <- cig_by_education %>% 
  as.data.frame()%>%
  group_by(binned_EDUC)%>%
  gather(`SMKCIGST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
  mutate(Prevalence = (Yes/(Yes + No))*100)
...
#
ecig_by_educDF <- ecig_by_education %>% 
  as.data.frame()%>%
  group_by(binned_EDUC)%>%
  gather(`SMKECIGST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
...
#
cigar_by_educDF <- cigar_by_education %>% 
  as.data.frame()%>%
  group_by(binned_EDUC)%>%
  gather(`CIGARST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
...
#
pipe_by_educDF <- pipe_by_education %>% 
  as.data.frame()%>%
  group_by(binned_EDUC)%>%
  gather(`PIPEST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
...
#
smkles_by_educDF <- smkles_by_education %>% 
  as.data.frame()%>%
  group_by(binned_EDUC)%>%
  gather(`SMOKELSST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
...
```{r}
cig_by_povDF <- cig_by_pov %>% 
  as.data.frame()%>%
  group_by(binned_RATCAT)%>%
  gather(`SMKCIGST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
  mutate(Prevalence = (Yes/(Yes + No))*100)
...
```{r}
cig_by_sexorientDF <- cig_by_sexorient %>% 
  as.data.frame()%>%
  group_by(ORIENT_A)%>%
  gather(`SMKCIGST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
...
#
ecig_by_sexorientDF <- ecig_by_sexorient %>% 
  as.data.frame()%>%
  group_by(ORIENT_A)%>%
  gather(`SMKECIGST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
...
#
cigar_by_sexorientDF <- cigar_by_sexorient %>% 
  as.data.frame()%>%
  group_by(ORIENT_A)%>%
  gather(`CIGARST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
...
#
pipe_by_sexorientDF <- pipe_by_sexorient %>% 
  as.data.frame()%>%
  group_by(ORIENT_A)%>%
  gather(`PIPEST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
...
#
smkles_by_sexorientDF <- smkles_by_sexorient %>% 
  as.data.frame()%>%
  group_by(ORIENT_A)%>%
  gather(`SMOKELSST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)%>%
...
cig_by_depress <- svytable(~DEPEV_A + SMKCIGST_A,
                           design = nhis19_design)
cig_by_depressDF <- cig_by_depress %>%
  as.data.frame()%>%
  filter(DEPEV_A == "Yes")%>%
  gather(`DEPEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`SMKCIGST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)
...
ecig_by_depress <- svytable(~DEPEV_A + SMKECIGST_A,
                           design = nhis19_design)
ecig_by_depressDF <- ecig_by_depress %>%
  as.data.frame()%>%
  filter(DEPEV_A == "Yes")%>%
  gather(`DEPEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`SMKECIGST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
...
cigar_by_depress <- svytable(~DEPEV_A + CIGARST_A,
                           design = nhis19_design)
cigar_by_depressDF <- cigar_by_depress %>%
  as.data.frame()%>%
  filter(DEPEV_A == "Yes")%>%
  gather(`DEPEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`CIGARST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
...
pipe_by_depress <- svytable(~DEPEV_A + PIPEST_A,
                           design = nhis19_design)
pipe_by_depressDF <- pipe_by_depress %>%
  as.data.frame()%>%
  filter(DEPEV_A == "Yes")%>%
  gather(`DEPEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`PIPEST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
...
smkles_by_depress <- svytable(~DEPEV_A + SMOKELSST_A,
                           design = nhis19_design)
smkles_by_depressDF <- smkles_by_depress %>%
  as.data.frame()%>%
  filter(DEPEV_A == "Yes")%>%
  gather(`DEPEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`SMOKELSST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
...
cig_by_anxiety <- svytable(~ANXEV_A + SMKCIGST_A,
                           design = nhis19_design)
cig_by_anxietyDF <- cig_by_anxiety %>%
  as.data.frame()%>%
  filter(ANXEV_A == "Yes")%>%
  gather(`ANXEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`SMKCIGST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)
...
ecig_by_anxiety <- svytable(~ANXEV_A + SMKECIGST_A,
                           design = nhis19_design)
ecig_by_anxietyDF <- ecig_by_anxiety %>%
  as.data.frame()%>%
  filter(ANXEV_A == "Yes")%>%
  gather(`ANXEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`SMKECIGST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
...
cigar_by_anxiety <- svytable(~ANXEV_A + CIGARST_A,
                           design = nhis19_design)
cigar_by_anxietyDF <- cigar_by_anxiety %>%
  as.data.frame()%>%
  filter(ANXEV_A == "Yes")%>%
  gather(`ANXEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`CIGARST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
...
pipe_by_anxiety <- svytable(~ANXEV_A + PIPEST_A,
                           design = nhis19_design)
pipe_by_anxietyDF <- pipe_by_anxiety %>%
  as.data.frame()%>%
  filter(ANXEV_A == "Yes")%>%
  gather(`ANXEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`PIPEST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
...
smkles_by_anxiety <- svytable(~ANXEV_A + SMOKELSST_A,
                           design = nhis19_design)
smkles_by_anxietyDF <- smkles_by_anxiety %>%
  as.data.frame()%>%
  filter(ANXEV_A == "Yes")%>%
  gather(`ANXEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`SMOKELSST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
...
cig_by_dementia <- svytable(~DEMENEV_A + SMKCIGST_A,
                           design = nhis19_design)
cig_by_dementiaDF <- cig_by_dementia %>%
  as.data.frame()%>%
  filter(DEMENEV_A == "Yes")%>%
  gather(`DEMENEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`SMKCIGST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
  spread(`Smoking Status`, Freq)
...
ecig_by_dementia <- svytable(~DEMENEV_A + SMKECIGST_A,
                           design = nhis19_design)
ecig_by_dementiaDF <- ecig_by_dementia %>%
  as.data.frame()%>%
  filter(DEMENEV_A == "Yes")%>%
  gather(`DEMENEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`SMKECIGST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
...
cigar_by_dementia <- svytable(~DEMENEV_A + CIGARST_A,
                           design = nhis19_design)
cigar_by_dementiaDF <- cigar_by_dementia %>%
  as.data.frame()%>%
  filter(DEMENEV_A == "Yes")%>%
  gather(`DEMENEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`CIGARST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
...
pipe_by_dementia <- svytable(~DEMENEV_A + PIPEST_A,
                           design = nhis19_design)
pipe_by_dementiaDF <- pipe_by_dementia %>%
  as.data.frame()%>%
  filter(DEMENEV_A == "Yes")%>%
  gather(`DEMENEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`PIPEST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
...
smkles_by_dementia <- svytable(~DEMENEV_A+ SMOKELSST_A,
                           design = nhis19_design)
smkles_by_dementiaDF <- smkles_by_dementia %>%
  as.data.frame()%>%
  filter(DEMENEV_A == "Yes")%>%
  gather(`DEMENEV_A`, key = "Mental Health", value = "Mental_hlth_status")%>%
  gather(`SMOKELSST_A`, key = "Type of Smoke", value = "Smoking Status")%>%
...
smoking_by_mentalhlth <- mutate(smoking_by_mentalhlth, Prevalence = (Yes/(Yes + No))*100)
...
cig_by_agedual <- cig_by_agedual%>%
  ...
  mutate(`Prevalence of cigarette` = (Yes/(Yes + No))*100)%>%
...
ecig_by_agedual <- ecig_by_agedual%>%
  ...
  mutate(`Prevalence of E-cigarette` = (Yes/(Yes + No))*100)%>%
...
cigar_by_agedual <- cigar_by_agedual%>%
  ...
  mutate(`Prevalence of Cigar` = (Yes/(Yes + No))*100)%>%
...
pipe_by_agedual <- pipe_by_agedual%>%
  ...
  mutate(`Prevalence of Pipe` = (Yes/(Yes + No))*100)%>%
...
smkles_by_agedual <- smkles_by_agedual%>%
  ...
  mutate(`Prevalence of Smokeless Tobacco` = (Yes/(Yes + No))*100)%>%
...
cig_ecig_by_agedual <- cig_ecig_by_agedual%>%
  ...
  mutate(`Dual prevalence cigaratte and e-cig` = (Yes/(Yes + No))*100)%>%
...
cig_pipe_by_agedual <- cig_pipe_by_agedual%>%
  ...
  mutate(`Dual prevalence cigaratte and pipe` = (Yes/(Yes + No))*100)%>%
...
cig_cigar_by_agedual <- cig_cigar_by_agedual%>%
  ...
  mutate(`Dual prevalence cigaratte and cigar` = (Yes/(Yes + No))*100)%>%
...
cig_smkles_by_agedual <- cig_smkles_by_agedual%>%
  ...
  mutate(`Dual prevalence cigaratte and smokeless tobacco` = (Yes/(Yes + No))*100)%>%
